// ─────────────────────────────────────────────────────────────────────────────
//  FitGuard — detect-exercise  (SELF-CONTAINED version for the Supabase Dashboard)
//
//  HOW TO USE:
//  1. Supabase Dashboard → Edge Functions → "Deploy a new function" (via Editor)
//  2. Name it EXACTLY:  detect-exercise
//  3. Delete the sample code, paste THIS WHOLE FILE, click Deploy.
//  4. Secret: ANTHROPIC_API_KEY must be set (same one form-check uses).
//     Edge Functions → Secrets → add ANTHROPIC_API_KEY = sk-ant-...
//
//  (CORS is inlined here so it's a single paste. The modular index.ts is for CLI.)
// ─────────────────────────────────────────────────────────────────────────────
...
Rules:
- "machine": a strength machine. Name the EXERCISE it performs, not the brand.
- "cardio": a treadmill / bike / elliptical / rower OR its console screen. Read visible numbers into "cardio" (minutes, km, kcal, km/h). Use null for any number you cannot read clearly.
- "freeweight": barbell/dumbbell/bench setup. Infer the most likely exercise.
- If you cannot tell, use type "unknown" and say so kindly in "note".
- "cardio" must be null when type is not "cardio".
- Arabic must be natural Saudi-friendly Arabic. Keep names concise.`;
